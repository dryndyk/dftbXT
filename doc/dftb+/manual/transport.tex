...
reservoirs and therefore quantum transport.  A new \kwcb{Transport}
...
\item[\is{WriteBinaryContact}] If set to \is{Yes}, the \verb|shiftcont_ContactId.bin| binary files are written
...
specifying \kw{Task = UploadContacts}, with possible additional option

\begin{ptable}
  \kw{ReadBinaryContact} & l & & No & \\
  \hline
\end{ptable}
%
\begin{description}
  \item[\is{ReadBinaryContact}] If set to \is{Yes}, the \verb|shiftcont_ContactId.bin| binary files are red
    instead of text format.  
\end{description}

{\bf Note:} if no task is specified, \dftbp{} assumes \is{UploadContacts} is the required task
in the transport block and the \verb|shiftcont_ContactId.dat| text files are used.

In order to set up a proper transport calculation the
...
\item[\iscb{TerminalCurrents}] in multi-terminal configurations is used to define
  ...
  3-4.
\item[\is{WriteTunn}] The energy-dependent transmission coefficients are calculated
  and written to the file {\em transmission.dat}.
\item[\is{WriteLDOS}] The energy-dependent local DOS is calculated and written to
  the file {\em localDOS.dat}.   
