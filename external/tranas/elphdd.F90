...
contains
  ...
  subroutine set_Gr(this, Gr, en_index, mix)
    class(ElPhonDephD) :: this
    ...
    do n=1,npl
      associate(pl_start=>this%struct%mat_PL_start(n),pl_end=>this%struct%mat_PL_end(n))
        forall(ii = 1:pl_end - pl_start + 1) 
          this%sigma_r(pl_start + ii - 1) = (1.0_dp-mix)*this%sigma_r(pl_start + ii - 1) &
          +mix*Gr(n,n)%val(ii,ii)*this%coupling(pl_start + ii - 1)
        end forall
      end associate
    end do

!    this%sigma_r = 0*this%Mixing*this%sigma_r !+(1-negf%inter%Mixing)*sigma_r_previous
    
  end subroutine set_Gr

  !> Give the Gn at given energy point to the interaction
  subroutine set_Gn(this, Gn, en_index, mix)
    class(ElPhonDephD) :: this
    ...
    do n=1,npl
      associate(pl_start=>this%struct%mat_PL_start(n),&
          & pl_end=>this%struct%mat_PL_end(n))
        forall(ii = 1:pl_end - pl_start + 1) 
          this%sigma_n(pl_start + ii - 1) = (1.0_dp-mix)*this%sigma_n(pl_start + ii - 1) & 
          +mix*Gn(n,n)%val(ii,ii)*this%coupling(pl_start + ii - 1)
        end forall
      end associate
    end do
