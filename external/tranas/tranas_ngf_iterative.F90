...
MODULE tranas_ngf_iterative
  ...
  use tranas_types_main, only : MAXNCONT, Tnegf, intarray
...
CONTAINS
  ...
  !------------------------------------------------------------------------------------------------!
  !> Driver for computing equilibrium retarded Green function (Outer blocks included)
  !! writing on memory
  !------------------------------------------------------------------------------------------------!
  ...
  subroutine calls_eq_mem_dns(pnegf,E,SelfEneR,Tlc,Tcl,gsurfR,A,outer)
    ...
    if (id0.and.pnegf%verbose.gt.130) print *, 'debug: tranas_ngf_mbngf.iterative: calls_eq_mem_dns is started'
    ...
    ! Add the retarded dephasing/interaction self-energies 
    call add_sigma_r(pnegf,ESH) !DAR

    ! Calculates the retarded Green function
    ...
    if (id0.and.pnegf%verbose.gt.130) print *, 'debug: tranas_ngf_mbngf.iterative: calls_eq_mem_dns is finished'
  ...
  SUBROUTINE calls_neq_elph(pnegf,E,SelfEneR,Tlc,Tcl,gsurfR,frm,Glout,out)
    ...
    !Work
    COMPLEX(dp) :: Ec
    INTEGER :: i,ierr,ncont,nbl,lbl,ii,n,m                              !DAR +ii,n
    INTEGER :: ref, iter
    INTEGER, DIMENSION(:), POINTER :: cblk, indblk
...
!    save Gr_previous !, Gn_previous !DAR
    ...
    if (id0.and.pnegf%verbose.gt.130) print *, 'debug: tranas_ngf_mbngf.iterative: calls_neq_elph is started'
 ...
 !   if(allocated(pnegf%inter).and.iter.eq.0) then
 !      if(.not.allocated(Gr_previous)) call allocate_blk_dns(Gr_previous,nbl)
 !     do n=1,nbl
      !do m=1,nbl
         !print *, n ,m,  'Gr(n,m)%nrow = ', Gr(n,m)%nrow    
 !        if(.not.allocated(Gr_previous(n,n)%val)) &
 !             allocate(Gr_previous(n,n)%val(Gr(n,n)%nrow,Gr(n,n)%nrow))
      !end do
 !     end do
 !   end if

 !   if(allocated(pnegf%inter).and.iter.gt.0) then
 !   do n=1,nbl
    !do m=1,nbl 
 !     Gr(n,n)%val(:,:) = pnegf%inter%Mixing*Gr(n,n)%val(:,:)+(1-pnegf%inter%Mixing)*Gr_previous(n,n)%val(:,:)
    !  !debug begin
    !  if (pnegf%verbose.gt.200) then
    !     print *, 'debug: Gr_previous(n,m)%val(:,:)'
    !     print *, 'debug: ', Gr_previous(n,m)%val(:,:)
    !     print *, 'debug: Gr(n,m)%val(:,:)'
    !     print *, 'debug: ', Gr(n,m)%val(:,:)
    !  end if
      !debug end
    !end do
 ...
 !   end if
  ...
  !  do n=1,nbl
    !do m=1,nbl    
  !    Gr_previous(n,n)%val(:,:) = Gr(n,n)%val(:,:)
    !end do
  !  end do
    ...
    !if(allocated(pnegf%inter).and.iter.eq.0) then
    !   if(.not.allocated(Gn_previous)) call allocate_blk_dns(Gn_previous,nbl)
    !  do n=1,nbl
    !  do m=1,nbl
    !     !print *, n ,m,  'Gr(n,m)%nrow = ', Gr(n,m)%nrow    
    !     if(.not.allocated(Gn_previous(n,m)%val)) &
    !          allocate(Gn_previous(n,m)%val(Gn(n,m)%nrow,Gn(n,m)%nrow))
    !  end do
    !  end do
    !end if
    ...
    !do n=1,nbl
    !do m=1,nbl 
    !   Gn(n,n)%val(:,:) = pnegf%inter%Mixing*Gn(n,n)%val(:,:)+(1-pnegf%inter%Mixing)*Gn_previous(n,n)%val(:,:)
    !end do
    !end do
    !end if
    
    !! Pass Gn to interaction model
    ...
    !do n=1,nbl
    !do m=1,nbl    
    !  Gn_previous(n,m)%val(:,:) = Gn(n,m)%val(:,:)
    !end do
    !end do
...
!!$ endif
    ...
    if (id0.and.pnegf%verbose.gt.130) print *, 'debug: tranas_ngf_mbngf.iterative: calls_neq_elph is finished'
  ...
  !------------------------------------------------------------------------------!  
  !DAR begin - add_sigma_r(pnegf,ESH)
  !------------------------------------------------------------------------------!
