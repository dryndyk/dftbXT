...
c contents:                                                            c
c----------                                                            c
c  readmt : reads matrices in the Boeing/Harwell format.        X      c
c  prtmt  : prints matrices in the Boeing/Harwell format.       X      c
...
c values= logical. indicates whether or not to print complex values.
...
c-----------------------------------------------------------------------
      ...
      external LENSTR
c local variables ---------------------------------------------------
      integer n,nr,nc,maxdim,istart,ilast,ii,k,ltit,nnz,m,kol,isep
...
c-----------------------------------------------------------------------
      ...
      real x, y, xnrow, ptsize, hscale, xwid, xht, xshift, yshift
      real vscale,tiny
      integer n,mode,nrow,nnz,maxdim,job,ips,ii,istart,ilast,k,iounit
c-------
      n = ncol
      if (mode .eq. 0) n = nrow
      nnz = ia(n+1) - ia(1) 
