...
c contents:                                                             c
c----------                                                             c
c zsubmat : extracts a submatrix from a sparse matrix.                  c
c zfilter : filters elements from a matrix according to their magnitude.c
c zfilterm: same as above, but for the MSR format                       c
c zcsort  : sorts the elements in increasing order of columns           c
c zclncsr : clean up the CSR format matrix, remove duplicate entry, etc c
c ztransp : in-place transposition routine (see also csrcsc in formats) c
c zcopmat : copy of a matrix into another matrix (both stored csr)      c
c zmsrcop : copies a matrix in MSR format into a matrix in MSR format   c
c zgetelm : returns a(i,j) for any (i,j) from a CSR-stored matrix.      c
c zgetdia : extracts a specified diagonal from a matrix.                c
c zgetl   : extracts lower triangular part                              c
c zgetu   : extracts upper triangular part                              c
c zlevels : gets the level scheduling structure for lower triangular    c
c          matrices.                                                    c
c zamask  : extracts     C = A mask M                                   c
c zrperm  : permutes the rows of a matrix (B = P A)                     c
c zcperm  : permutes the columns of a matrix (B = A Q)                  c
c zdperm  : permutes both the rows and columns of a matrix (B = P A Q ) c
c zdperm1 : general extractiob routine (extracts arbitrary rows)        c
c zdperm2 : general submatrix permutation/extraction routine            c
c zdmperm : symmetric permutation of row and column (B=PAP') in MSR fmt c
c zdvperm : permutes a real vector (in-place)                           c
c zivperm : permutes an integer vector (in-place)                       c
c zretmx  : returns the max absolute value in each row of the matrix    c
c zdiapos : returns the positions of the diagonal elements in A.        c
c zextbdg : extracts the main diagonal blocks of a matrix.              c
c zgetbwd : returns the bandwidth information on a matrix.              c
c zblkfnd : finds the block-size of a matrix.                           c
c zblkchk : checks whether a given integer is the block size of A.      c
c zinfdia : obtains information on the diagonals of A.                  c
c zamubdg : gets number of nonzeros in each row of A*B (as well as NNZ) c 
c zaplbdg : gets number of nonzeros in each row of A+B (as well as NNZ) c
c zrnrms  : computes the norms of the rows of A                         c
c zcnrms  : computes the norms of the columns of A                      c
c zroscal : scales the rows of a matrix by their norms.                 c
c zcoscal : scales the columns of a matrix by their norms.              c
c zaddblk : Adds a matrix B into a block of A.                          c
c zget1up : Collects the first elements of each row of the upper        c
c           triangular portion of the matrix.                           c
c zxtrows : extracts given rows from a matrix in CSR format.            c
c zcsrkvstr:  Finds block row partitioning of matrix in CSR format      c
c zcsrkvstc:  Finds block column partitioning of matrix in CSR format   c
c zkvstmerge: Merges block partitionings, for conformal row/col pattern c
c-----------------------------------------------------------------------c
...
c 
         call zivperm (nnz, ja(ifirst), iwork) 
...
c----------------------------------------------------------------------- 
      subroutine zlevels (n, jal, ial, nlev, lev, ilev, levnum)
...
c-----------------------------------------------------------------------  
      subroutine zretmx(n,a,ja,ia,dd)
      complex*16 a(*)
      real*8 dd(*)
      integer n,ia(*),ja(*)
c-----------------------------------------------------------------------
...
c dd	= complex*16 array of length n. On output,entry dd(i) contains
...
c----------------------------------------------------------------------- 
      subroutine zdiapos  (n,ja,ia,idiag) 
...
c-----------------------------------------------------------------------
      subroutine zdscaldg (n,a,ja,ia,diag,job)
      complex*16 a(*)
      real*8 diag(*), t
      integer ia(*),ja(*)
c----------------------------------------------------------------------- 
...
c----------------------------------------------------------------------c
 ...
 10   do 110 j=1,n
         ...
         t = 0.0d0
         do 111 k = k1,k2
 111        t = t+conjg(a(k))*a(k)
 110        diag(j) = sqrt(t)
            goto 12
 11   continue
      call zretmx (n,a,ja,ia,diag)
c------
 12   do 1 j=1,n
...
c----------------------------------------------------------------------- 
      subroutine zblkfnd (nrow,ja,ia,nblk)
c-----------------------------------------------------------------------
...
c     
         call zblkchk (nrow,ja,ia,iblk,imsg)	       
         if (imsg .eq. 0) then 
c     
c     block size found
c     
...
c-----------------------------------------------------------------------
      subroutine zblkchk (nrow,ja,ia,nblk,imsg)
c----------------------------------------------------------------------- 
...
c-----------------------------------------------------------------------
      subroutine zinfdia (n,ja,ia,ind,idiag) 
      integer ia(*), ind(*), ja(*)
c-----------------------------------------------------------------------
...
c----------------------------------------------------------------------- 
      subroutine zamubdg (nrow,ncol,ncolb,ja,ia,jb,ib,ndegr,nnz,iw) 
...
c----------------------------------------------------------------------- 
      subroutine zaplbdg (nrow,ncol,ja,ia,jb,ib,ndegr,nnz,iw) 
...
c-----------------------------------------------------------------
      ...
      do 1 j=1, nrow
         if (diag(j) .eq. 0.0d0) then
            ...
            zdiag(j) = cmplx(1.0d0/diag(j))
         endif
 1    continue
      call zdiamua(nrow,job,a,ja,ia,zdiag,b,jb,ib)
      return
c-------end-of-roscal---------------------------------------------------
c-----------------------------------------------------------------------
      end
...
c-----------------------------------------------------------------
 ...
 1    continue
      call zamudia (nrow,job,a,ja,ia,zdiag,b,jb,ib)
      return
c--------end-of-coscal-------------------------------------------------- 
c-----------------------------------------------------------------------
      end
...
c-----------------------------------------------------------------------
      subroutine zcsrkvstr(n, ia, ja, nr, kvstr)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
c------------------------end-of-csrkvstr--------------------------------
      subroutine zcsrkvstc(n, ia, ja, nc, kvstc, iwk)
c-----------------------------------------------------------------------
...
c------------------------end-of-csrkvstc--------------------------------
c-----------------------------------------------------------------------
      subroutine zkvstmerge(nr, kvstr, nc, kvstc, n, kvst)
c-----------------------------------------------------------------------
