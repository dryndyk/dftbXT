...
c contents:                                                            c
c----------                                                            c
c zcsrdns  : converts a row-stored sparse matrix into the dense format. c
c zdnscsr  : converts a dense matrix to a sparse storage format.        c
c zcoocsr  : converts coordinate to  to csr format                      c
c zcoicsr  : in-place conversion of coordinate to csr format            c
...
c           row format                                                 c
c zmsrcsr  : converts modified sparse row format to compressed sparse   c
c           row format.                                                c
c zcsrcsc  : converts compressed sparse row format to compressed sparse c
c           column format (transposition)                              c
c zcsrcsc2 : rectangular version of csrcsc                              c
c zcsrlnk  : converts compressed sparse row to linked list format       c
c zlnkcsr  : converts linked list format to compressed sparse row fmt   c
c zcsrdia  : converts a compressed sparse row format into a diagonal    c
c           format.                                                    c
c zdiacsr  : converts a diagonal format into a compressed sparse row    c
c           format.                                                    c
c zbsrcsr  : converts a block-row sparse format into a compressed       c
c           sparse row format.                                         c
c zcsrbsr  : converts a compressed sparse row format into a block-row   c
c           sparse format.                                             c
c zcsrbnd  : converts a compressed sparse row format into a banded      c
c           format (linpack style).                                    c
c zbndcsr  : converts a banded format (linpack style) into a compressed c
c           sparse row storage.                                        c
c zcsrssk  : converts the compressed sparse row format to the symmetric c
c           skyline format                                             c
c zsskssr  : converts symmetric skyline format to symmetric  sparse row c
c           format.                                                    c
c zcsrjad  : converts the csr format into the jagged diagonal format    c
c zjadcsr  : converts the jagged-diagonal format into the csr format    c
...
c------------------------------------------------------------------------
c this subroutine zconverts a matrix stored in coordinate format into 
...
c--------need to zero workspace; first find ncol
         ...
         do i = 1, ncol
            iwk(i) = 0
         enddo
         call zcsrkvstr(n, ia, ja, nr, kvstr)
         call zcsrkvstc(n, ia, ja, nc, kvstc, iwk)
      endif
...
c        use iwk temporarily
         call zkvstmerge(nr, kvstr, nc, kvstc, nb, iwk)
         nr = nb
         nc = nb
         do i = 1, nb+1
            kvstr(i) = iwk(i)
