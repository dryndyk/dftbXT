...
contains
  ...
  subroutine runDftbPlus(env, input) !!DAR!! + input
    ...
    type(TInputData), intent(in) :: input !!DAR!!
    ...
    geoOpt: do iGeoStep = 0, nGeoSteps
      ...
      lpDets : do iDet = 1, nDets
        ...
        call processGeometry(env, iGeoStep, iLatGeoStep, tWriteRestart, tStopScc, tExitGeoOpt, input) !!DAR!! + input)
    ...
    #:endif
      ...
      if (tWriteCharges) then
        call writeCharges(fCharges, tWriteChrgAscii, orb, qInput, qBlockIn, qiBlockIn, deltaRhoIn, input%ctrl%verbose)
      end if

      if (tForces) then
        call getNextGeometry(env, iGeoStep, tWriteRestart, constrLatDerivs, tCoordStep, tGeomEnd,&
            & tStopDriver, iLatGeoStep, tempIon, tExitGeoOpt, input)
        if (tExitGeoOpt) then
          exit geoOpt
        end if
      end if
...
#:endif
    ...
    if (tWriteAutotest.and..not.tRestartNoSC) then
      ...
      call writeAutotestTag(autotestTag, electronicSolver, tPeriodic, cellVol, tMulliken, qOutput,&
          ...
          & taggedWriter, tunnTot, ldosTot, lCurrArray)
    end if
    ...
    REKS_SCC: if (allocated(reks)) then
      ...
      if(tSccCalc.or.(.not.tTunn)) then !DAR
        ...
        call getDensity(env, iSccIter, denseDesc, ham, over, neighbourList, nNeighbourSk,&
            ...
            & tLargeDenseMatrices, deltaDftb, input)  !!DAR!! + input
      ...
      if (tSccCalc.and.(.not.((iSccIter.eq.maxSccIter).and.input%ctrl%tNoLastDensity))) then !DAR
          ...
          if(input%ctrl%verbose.gt.30) call printSccInfo(tDftbU, iSccIter, dftbEnergy(deltaDftb%iDeterminant)%Eelec, diffElec,&
              & sccErrorQ)
    ...
    if (tForces) then
      ...
      if (allocated(reks)) then
        ...
        call getEnergyWeightedDensity(env, electronicSolver, denseDesc, forceType, filling, eigen,&
          ...
          & iCellVec, cellVec, tRealHS, ham, over, parallelKS, tHelical, species, coord, iSCCIter, mu, ERhoPrim,&
          & eigvecsReal, SSqrReal, eigvecsCplx, SSqrCplx, input) !!DAR!! + input  
  ...
  subroutine getNextGeometry(env, iGeoStep, tWriteRestart, constrLatDerivs, tCoordStep, tGeomEnd,&
      & tStopDriver, iLatGeoStep, tempIon, tExitGeoOpt, input) !!DAR!! + input)
...
#:endif    
    ...
    #:if WITH_MPI
      call calcdensity_green(0, env%mpi%globalComm, parallelKS%localKS, ham, over, &
          ...
          & tempElec, kPoint, kWeight, rhoPrim, energy%Eband, Ef, energy%E0, energy%TS, mu, input%ginfo%tundos) !!DAR!!      
    #:else
      call calcdensity_green(0, parallelKS%localKS, ham, over, &
          & descHS, neighbourlist%iNeighbour, nNeighbourSK, denseDesc%iAtomStart, iSparseStart, &
          & img2CentCell, iCellVec, cellVec, orb, nEl, & 
          & tempElec, kPoint, kWeight, rhoPrim, energy%Eband, Ef, energy%E0, energy%TS, mu, input%ginfo%tundos) !!DAR!! 
    #:endif
!NEW      call calcdensity_green(iSCC, env, parallelKS%localKS, ham, over, neighbourlist%iNeighbour,&
!NEW      & nNeighbourSK, denseDesc%iAtomStart, iSparseStart, img2CentCell, iCellVec, cellVec, orb,&
!NEW      & kPoint, kWeight, mu, rhoPrim, energy%Eband, Ef, energy%E0, energy%TS)
    #:else
  ...
  subroutine getEnergyWeightedDensity(env, electronicSolver, denseDesc, forceType, filling,&
      ...
      & ERhoPrim, HSqrReal, SSqrReal, HSqrCplx, SSqrCplx, input) !!DAR!! + input
...
!NEW            & img2CentCell, iCellVec, cellVec, orb, kPoint, kWeight, mu, ERhoPrim)
     #:else
       call calcEdensity_green(0, parallelKS%localKS, ham, over, &
          & neighbourlist%iNeighbour, nNeighbourSK, denseDesc%iAtomStart, iSparseStart, &
          & img2CentCell, iCellVec, cellVec, orb, & 
          & kPoint, kWeight, ERhoPrim, mu, input%ginfo%tundos) !!DAR!!
!NEW        call calcEdensity_green(iSCC, parallelKS%localKS, ham, over,&
!NEW            & neighbourlist%iNeighbour, nNeighbourSK, denseDesc%iAtomStart, iSparseStart,&
!NEW            & img2CentCell, iCellVec, cellVec, orb, kPoint, kWeight, mu, ERhoPrim) 
    #:endif
      end if
