...
#:endif
  ...
  subroutine processGeometry(env, iGeoStep, iLatGeoStep, tWriteRestart, tStopScc,&
      & tExitGeoOpt, input, stat) !!DAR!! + input)
    ...
    REKS_SCC: if (allocated(reks)) then
      ...
      if(tSccCalc.or.(.not.tTunn)) then !DAR
        ...
        call getDensity(env, iSccIter, denseDesc, ham, over, neighbourList, nNeighbourSK,&
          ...
          & deltaRhoOutSqr, qOutput, nNeighbourLC, tLargeDenseMatrices, input)  !!DAR!! + input
    ...
    if (input%ctrl%verbose.gt.0) call printEnergies(energy, electronicSolver)
...
#:endif
  ...
  subroutine getDensity(env, iScc, denseDesc, ham, over, neighbourList, nNeighbourSK, iSparseStart,&
      ...
      & nNeighbourLC, tLargeDenseMatrices, input)  !!DAR!! + input)
...
!NEW          & img2CentCell, iCellVec, cellVec, orb, kPoint, kWeight, mu, rhoPrim, Eband, Ef, E0, TS)
    #:else
      call calcdensity_green(0, parallelKS%localKS, ham, over, &
          ...
          & img2CentCell, iCellVec, cellVec, orb, nEl, & 
          & tempElec, kPoint, kWeight, rhoPrim, Eband, Ef, E0, TS, mu, input%ginfo%tundos) !!DAR!! 
!NEW      call calcdensity_green(iSCC, parallelKS%localKS, ham, over,&
!NEW          & neighbourlist%iNeighbour, nNeighbourSK, denseDesc%iAtomStart, iSparseStart,&
!NEW          & img2CentCell, iCellVec, cellVec, orb, kPoint, kWeight, mu, rhoPrim, Eband, Ef, E0, TS)
    #:endif
    #:else
